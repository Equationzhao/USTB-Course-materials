...
namespace sf
{
	...
	class contention_free_shared_mutex
	{
		...
		using array_slock_t = std::array<cont_free_flag_t, contention_free_count>;
		...
		auto get_fast_this_thread_id() -> std::thread::id { return std::this_thread::get_id(); }
		...
		struct unregister_t
		{
			...
			unregister_t(int index, const std::shared_ptr<array_slock_t>& ptr) : thread_index(index),
				array_slock_ptr(ptr)
			{
			}

			unregister_t(unregister_t&& src) noexcept : thread_index(src.thread_index),
		...
		auto get_or_set_index(index_op_t index_op = get_index_op, int set_index = -1) -> int
		{
	...
	public:
		contention_free_shared_mutex() :
			...
			shared_locks_array(*shared_locks_array_ptr), avoid_falsesharing_2{}, recursive_xlock_count(0),
		...
		auto unregister_thread() -> bool { return get_or_set_index(unregister_thread_op) >= 0; }
		...
		auto register_thread() -> int
		{
			...
			if (cur_index == -1)
			{
				if (shared_locks_array_ptr.use_count() <= static_cast<int>(shared_locks_array.size()))
		...
		{
			const int register_index = register_thread();
		...
		auto unlock_shared() -> void
		{
			...
			if (register_index >= 0)
			{
				const int recursion_depth = shared_locks_array[register_index].value.load(std::memory_order_acquire);
				assert(recursion_depth > 1);
		...
		auto lock() -> void
		{
			...
			{
				size_t i = 0;
				for (bool flag = false; !want_x_lock.compare_exchange_weak(flag, true, std::memory_order_seq_cst); flag
				     = false)
